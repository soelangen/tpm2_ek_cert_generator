...
readonly owner_auth='-w root' # set to '-w $PASSWORD' if different from null
...
tpm2_nvdefine "${h_ek_pub_crt}" -s "${ek_der_cert_size}" -a "${ek_cert_nvram_attr}" -C o -P root
tpm2_nvwrite "${h_ek_pub_crt}" -i "${output_der_crt}" -C o -P root
