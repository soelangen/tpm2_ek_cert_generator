...
readonly ek_alg='rsa'
readonly endorsment_auth='root' # set to '-P $PASSWORD' if different from null
readonly owner_auth='root' # set to '-w $PASSWORD' if different from null
...
tpm2_createek -G "${ek_alg}" -u "${working_dir}public.ek.portion" -c "${h_ek_pub_key}" -P ${endorsment_auth} -w ${owner_auth}
...
tpm2_nvdefine "${h_ek_pub_crt}" -C o -s "${ek_der_cert_size}" -a "${ek_cert_nvram_attr}"
